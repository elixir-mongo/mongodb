defmodule Mongo.Connection.Auth.CR do
  ...
  def auth({database, username, password}, s) do
    case sync_command(-2, database, [getnonce: 1], s) do
      {:ok, %{"nonce" => nonce, "ok" => 1.0}} ->
        nonce(nonce, database, username, password, s)
      {:tcp_error, _} = error ->
        error
    end
  end

  defp nonce(nonce, database, username, password, s) do
    digest = digest(nonce, username, password)
    command = [authenticate: 1, user: username, nonce: nonce, key: digest]

    case sync_command(-3, database, command, s) do
      {:ok, %{"ok" => 1.0}} ->
        :ok
