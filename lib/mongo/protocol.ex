defmodule Mongo.Protocol do
    ...
    case result do
      ...
      {:disconnect, reason, s} ->
        reason =
          case reason do
            {:tcp_recv, reason} ->
              Mongo.Error.exception(tag: :tcp, action: "recv", reason: reason, host: s.host)

            {:tcp_send, reason} ->
              Mongo.Error.exception(tag: :tcp, action: "send", reason: reason, host: s.host)
  ...
  defp ssl(%{socket: {:gen_tcp, sock}} = s, opts) do
    ...
    case :ssl.connect(sock, ssl_opts, s.connect_timeout_ms) do
      ...
      {:error, reason} ->
        ...
        {:error,
         Mongo.Error.exception(tag: :ssl, action: "connect", reason: reason, host: s.host)}
    end
  end
  ...
  defp tcp_connect(opts, s) do
    ...
    case :gen_tcp.connect(host, port, sock_opts, s.connect_timeout_ms) do
      ...
      {:error, reason} ->
        {:error,
         Mongo.Error.exception(tag: :tcp, action: "connect", reason: reason, host: s.host)}
    end
  end
  ...
  def handle_execute(
    ...
    tmp_state = %{
      original_state
      | database: Keyword.get(opts, :database, original_state.database)
    }
  ...
  defp handle_execute(:find, coll, [query, select], opts, s) do
    ...
    op_query(
      coll: Utils.namespace(coll, s, opts[:database]),
      query: query,
      select: select,
      num_skip: num_skip,
  ...
  defp handle_execute(:command, nil, [query], opts, s) do
    ...
    op_query(
      coll: Utils.namespace("$cmd", s, opts[:database]),
      query: query,
      select: "",
      num_skip: 0,
      num_return: 1,
